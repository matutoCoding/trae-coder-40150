...
export const useAppStore = create<AppStore>((set, get) => ({
  ...
  previewBills: (periodStart: string, periodEnd: string): BillPreviewResult => {
    const store = get();
    const { pricing, contracts, storageUnits, accessGrants, tenants } = store;
    if (!pricing) return { items: [], billCount: 0, totalAmount: 0, frozenCount: 0 };
    ...
    for (const [tenantId, tenantContracts] of contractsByTenant) {
      ...
      if (tenantTotal > pricing.overdueFreezeThreshold) {
        tenantsToFreeze.push(tenantId);
      }
    }
