...
// 从 localStorage 加载所有集合数据
function loadAllData() {
  initDB();
  return {
    tiers: getCollection<TenantTier>(COLLECTION_KEYS.tiers),
    tenants: getCollection<Tenant>(COLLECTION_KEYS.tenants),
    quotaLedgers: getCollection<QuotaLedger>(COLLECTION_KEYS.quotaLedgers),
    tierChangeRecords: getCollection<TierChangeRecord>(COLLECTION_KEYS.tierChangeRecords),
    pricingRule: getCollection<PricingRule>(COLLECTION_KEYS.pricingRule)[0] ?? null,
    storageUnits: getCollection<StorageUnit>(COLLECTION_KEYS.storageUnits),
    contracts: getCollection<RentalContract>(COLLECTION_KEYS.contracts),
    bills: getCollection<Bill>(COLLECTION_KEYS.bills),
    accessGrants: getCollection<AccessGrant>(COLLECTION_KEYS.accessGrants),
    auditLogs: getCollection<AuditLog>(COLLECTION_KEYS.auditLogs),
  };
}
...
interface AppStoreActions {
  // 初始化时调用
  initData: () => void;
  // 重新从 localStorage 加载所有数据
  refreshData: () => void;
  // 等级相关
  getTierById: (id: string) => TenantTier | undefined;
...
export const useAppStore = create<AppStore>((set, get) => ({
  ...
  updatePricing: (partial: Partial<PricingRule>) => {
    const store = get();
    if (!store.pricingRule) return;
    const before = { ...store.pricingRule };
    const updated = update<PricingRule>(
      COLLECTION_KEYS.pricingRule,
      store.pricingRule.id,
      {
        ...partial,
        updatedAt: nowStr(),
  ...
  generateBills: (periodStart: string, periodEnd: string) => {
    const store = get();
    const { pricingRule, contracts, storageUnits, accessGrants } = store;
    ...
    for (const [tenantId, tenantContracts] of contractsByTenant) {
      ...
      // 检查是否超过逾期冻结阈值
      if (totalAmount > pricingRule.overdueFreezeThreshold) {
      ...
      for (let i = 0; i < updatedAccessGrants.length; i++) {
        ...
        if (
          ...
          const updated = update<AccessGrant>(
            ...
            {
              status: 'frozen',
              frozenReason: `账单金额超过冻结阈值 ${pricingRule.overdueFreezeThreshold} 元`,
            }
          );
